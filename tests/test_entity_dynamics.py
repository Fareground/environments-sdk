...
def test_reads_world_values_and_world_physics_per_entity():
    ...
    contract["physics"]["vars"] = {"heat": {"start": 1, "rate": "0"}}
    contract["physics"]["per"]["cell"] = {"params": {"scale": "$world.temperature"},
                                          "read": {"boost": "$it.k * 10"},
...
def test_where_limits_which_entities_integrate_this_step():
    contract = copy.deepcopy(DECAY)
    contract["physics"]["per"]["cell"]["where"] = "$it.k > 0.2"
...
def test_world_variables_take_noise_too():
    ...
    assert _errors({**contract, "physics": {"vars": {"p": {"start": 1, "noise": "1"}}}, "outputs": {}}) == [
        ("physics.vars.p.noise", "noise needs a rate")]
...
def test_a_run_split_by_a_snapshot_ends_exactly_like_a_straight_run():
    ...
    contract["physics"]["vars"] = {"drift": {"start": 0, "rate": "1", "noise": "0.3"}}
    contract["physics"]["per"]["particle"]["vars"]["x"]["rate"] = "drift * 0.01"
...
def test_the_checker_names_every_problem_with_per_entity_dynamics():
    ...
    expected = [
        ("physics.per.cell", "'scale' is both a world physics name and a param, so the math cannot tell them apart"),
        ("physics.per.cell", "'pi' is a math function, constant or the time t"),
        ("physics.per.cell.vars.label", "'label' is a text property; integrated variables are numbers"),
        ("physics.per.cell.vars.missing", "'cell' has no property 'missing'"),
        ("physics.per.cell.vars.count", "'count' is a int property; integrated variables are numbers"),
        ("physics.per.cell.write.load", "'load' is integrated, so a write would overwrite it"),
        ("physics.per.cell.write.note", "'note' is a text property; physics math gives numbers"),
        ("physics.per.hot_cell.vars.load", "is also integrated by physics.per.cell, which covers every hot_cell"),
        ("physics.per.ghost", "'ghost' is not a declared type"),
    ]
    for issue in expected:
        assert issue in found, (issue, found)
    assert any(path == "physics.per.cell.read.heat" for path, _ in found)
    assert any(path == "physics.per.hot_cell.vars.load.rate" and "unknown_name" in message for path, message in found)
