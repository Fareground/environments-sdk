...
def test_the_link_effect_creates_with_defaults_and_updates_only_what_it_names():
    ...
    assert isinstance(made["note"], fg_env.sdk.expr.Untrusted)
...
def test_links_lists_outgoing_links_for_views_and_rules():
    ...
    scope = world.scope()
    count = fg_env.sdk.expr.compile_expr("$count($links(ana, knows, $it.met != 'x'))")(scope)
    assert count == 2
...
def test_a_run_split_by_a_snapshot_keeps_every_link_field_and_its_provenance():
    ...
    assert isinstance(restored.world.link_view("ana", "cy", "trusts").expr_attr("note", None), fg_env.sdk.expr.Untrusted)
