...
@pytest.mark.parametrize("root", ["viewer", "actor"])
def test_event_expressions_follow_the_reader_even_without_an_explicit_viewer(root):
    env, _ = run()
    for who, expected in [("a", 1), ("b", 0)]:
        scope = env.world.scope(**{root: env.world.entities[who]})
    ...
    assert evaluate("$events(record)", scope) == []  # the explicit reader takes precedence
