...
def test_every_search_method_finds_the_known_peak_of_a_noisy_hill(method, tolerance):
    result = fg_env.optimise(HILL, {"x": {}, "y": {"step": 1}}, "maximise profit", runs=4, budget=121, method=method)
...
def test_continuous_decisions_converge_to_the_known_optimum(method):
    result = fg_env.optimise(BOWL, {"a": {}, "b": {}}, "maximise f", runs=4, budget=150, method=method)
...
def test_auto_picks_a_grid_when_it_fits_the_budget_a_simplex_for_continuous_decisions_and_local_search_otherwise():
    assert fg_env.optimise(HILL, {"x": {}}, "maximise profit", runs=2).method == "grid"
    assert fg_env.optimise(BOWL, {"a": {}, "b": {}}, "maximise f", runs=2, budget=30).method == "nelder_mead"
    stepped = fg_env.optimise(HILL, {"x": {}, "y": {"step": 0.1}}, "maximise profit", runs=2, budget=60)
...
def test_a_vector_and_a_choice_are_searched_together_to_their_known_best():
    result = fg_env.optimise(HILL, {"v": {"length": 3, "low": 0, "high": 8, "step": 1}, "mode": {}}, "maximise vec",
                             runs=1, budget=200)
...
def test_a_vector_with_a_fixed_sum_or_a_monotone_order_only_ever_tries_decisions_with_that_structure():
    fixed = fg_env.optimise(HILL, {"v": {"length": 3, "low": 0, "high": 9, "step": 1, "sum": 9}}, "maximise even",
                            runs=1, budget=100)
    assert fixed.best == {"v": [3, 3, 3]} and all(sum(h["decision"]["v"]) == 9 for h in fixed.history)
    falling = fg_env.optimise(HILL, {"v": {"length": 3, "low": 0, "high": 9, "step": 1, "monotone": "decreasing"}},
...
def test_a_vector_takes_bounds_per_position_and_a_local_search_starts_where_it_is_told():
    result = fg_env.optimise(HILL, {"v": {"length": 3, "low": [0, 4, 0], "high": [1, 9, 9], "step": 1,
...
def test_the_cheapest_decision_that_meets_a_constraint_is_chosen_and_its_neighbours_show_why():
    result = fg_env.optimise(HILL, {"x": {}, "y": {"step": 1}}, "minimise cost", ["service >= 9"], runs=2)
...
def test_an_unmeetable_constraint_reports_the_closest_decision_and_how_far_it_misses():
    result = fg_env.optimise(HILL, {"x": {}, "y": {"step": 1}}, "minimise cost", ["service >= 50"], runs=2)
...
def test_a_share_of_runs_constraint_is_the_share_of_runs_where_it_holds_with_a_wilson_interval():
    result = fg_env.optimise(HILL, {"x": {"low": 0, "high": 2}}, "minimise cost",
...
def test_a_risk_averse_percentile_objective_prefers_the_safe_plan_the_mean_passes_over():
    assert fg_env.optimise(RISK, {"plan": {}}, "maximise gain", runs=40).best == {"plan": "risky"}
    assert fg_env.optimise(RISK, {"plan": {}}, "maximise p10 of gain", runs=40).best == {"plan": "safe"}


def test_an_objective_can_be_an_expression_over_outputs_and_inputs():
    result = fg_env.optimise(HILL, {"x": {}}, "maximise $outputs.profit - 10 * $inputs.x", inputs={"y": 7}, runs=2)
...
def test_fresh_seeds_confirm_a_real_winner_and_call_a_lucky_one_within_noise():
    real = fg_env.optimise(NOISE, {"k": {}}, "maximise peaked", runs=3, holdout_seeds=30)
    assert real.best == {"k": 7} and real.holdout["still_wins"] and real.holdout["difference"]["clear"]
    luck = fg_env.optimise(NOISE, {"k": {}}, "maximise luck", runs=3, holdout_seeds=30)
...
def test_the_same_seed_reproduces_the_whole_search_with_or_without_worker_processes():
    ...
    alone = fg_env.optimise(HILL, {"x": {}, "y": {"step": 0.5}}, "maximise profit", ["service >= 8"], **kwargs)
    pooled = fg_env.optimise(HILL, {"x": {}, "y": {"step": 0.5}}, "maximise profit", ["service >= 8"], workers=2,
                             **kwargs)
    assert alone.to_dict() == pooled.to_dict()
    other = fg_env.optimise(HILL, {"x": {}, "y": {"step": 0.5}}, "maximise profit", ["service >= 8"],
                            **{**kwargs, "seed": 12})
    assert other.estimates != alone.estimates


def test_uncertain_parameters_make_the_decision_robust_to_not_knowing_them():
    known = fg_env.optimise(BOWL, {"a": {"low": 0, "high": 5, "step": 0.5}}, "maximise p10 of robust", runs=20)
    assert known.best == {"a": 4.0}
    robust = fg_env.optimise(BOWL, {"a": {"low": 0, "high": 5, "step": 0.5}}, "maximise p10 of robust", runs=20,
                             uncertainty=[{"b": 1.0}, {"b": 4.0}])
    assert robust.best == {"a": 2.5}
    with pytest.raises(ValueError, match="also drawn from uncertainty"):
        fg_env.optimise(BOWL, {"a": {"step": 1}}, "maximise f", inputs={"b": 2}, uncertainty=[{"b": 1.0}], runs=2)


def test_two_objectives_trace_a_pareto_frontier_of_undominated_decisions():
    result = fg_env.optimise(HILL, {"x": {}, "y": {"step": 1}}, ["minimise cost", "maximise service"], runs=2,
                             budget=121)
...
def test_mistakes_say_what_to_fix(decisions, objective, constraints, options, message):
    with pytest.raises(ValueError, match=message):
        fg_env.optimise(HILL, decisions, objective, constraints, runs=1, **options)
...
def test_the_guide_has_an_optimise_part_on_the_core_guides_map():
    ...
    assert "fg_env.optimise(" in part and "in 90% of runs" in part and "Pareto" in part
    core = fg_env.guide()
