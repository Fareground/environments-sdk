...
def contract(surface, probability, dynamic=False):
    ...
    if surface == 'action':
        ...
        c['events'] = [{'chance': value, 'do': '$world.converted += 1'}]
    elif surface == 'policy':
...
@pytest.mark.parametrize('surface,path', [('action', 'actions.attempt.chance'), ('event', 'events[0].chance'),
                                         ('policy', 'policies.p.rules[0].chance')])
...
def test_nested_random_probability_preserves_existing_draw_order(seed):
    from fg_env.sdk.seeds import SeedTree
    rng = SeedTree(seed).rng('world')
