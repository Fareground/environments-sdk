...
@pytest.mark.parametrize("error", [PromptTooLong("prompt is too long: 250000 tokens > 200000 maximum"),
                                   RequestTooLarge("request_too_large: Request exceeds the maximum size")])
def test_a_prompt_too_long_for_the_model_is_diagnosed_as_such_not_as_a_failing_provider(error):
    """No retry can shorten a prompt: the finding says so and points at what the agent reads (audit 9 LLM M1); a 413
    (Anthropic's `request_too_large`) is one too (audit 14 agentif LOW-5)."""
    client = FailingAnthropic([], [error] * 20)
