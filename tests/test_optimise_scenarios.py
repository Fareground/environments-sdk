...
STORE = Path(__file__).parents[1] / "examples" / "contracts" / "auto_parts_store.json"
...
def test_a_plan_on_four_seeds_cannot_be_shown_with_confidence_stays_borderline_and_flags_luck():
    few = fg_env.analysis.optimise(CENTRE, STAFFING, "minimise staffing_cost", ["sl >= 0.8 in 100% of runs"], runs=4,
                                   budget=24,
                          holdout_seeds=20)
...
def test_a_plan_picked_on_four_seeds_is_confirmed_on_more_seeds_until_its_share_of_runs_holds_with_confidence():
    few = fg_env.analysis.optimise(CENTRE, STAFFING, "minimise staffing_cost", ["sl >= 0.8 in 80% of runs"], runs=4,
                                   budget=24,
                          holdout_seeds=20)
...
def test_a_service_level_per_category_keeps_the_fill_rate_with_less_stock_across_the_fitted_uncertainty():
    ...
    # with the search's margin), so a plan confident on four runs keeps 86% of the uniform stock where one on eight
    # keeps 77% — both confident on 24 fresh seeds (lower 90% bounds 98.6% and 98.0% against 97.5%)
    decision = {"service_level_by_category": {"keys": categories, "low": 0.5, "high": 0.99, "step": 0.05,
                                              "start": {c: 0.95 for c in categories}}}
    result = fg_env.analysis.optimise(STORE, decision, "minimise reorder_average_stock_value",
                                      ["shop_fill_rate >= 0.975"], runs=8, budget=16, workers=2,
                                      inputs={"policy": "service", "parameter_uncertainty": 0}, **common)
    assert result.feasible and {"growth_rate", "promo_lift", "lead_noise_mean"} <= set(priors)

    def fresh(levels):
        inputs = {"policy": "service", "parameter_uncertainty": 0, **levels}
        runs = fg_env.experiment(STORE, runs=24, seed=9090, arms=[None], **common, inputs=inputs).arms["baseline"].runs
        return (fmean(r.outputs["shop_fill_rate"] for r in runs),
                fmean(r.outputs["reorder_average_stock_value"] for r in runs))
...
def test_prices_per_grade_stay_in_grade_order_and_beat_listing_at_the_fair_price_on_fresh_seeds():
    ...
    result = fg_env.analysis.optimise(PHONES,
                                      {"price": {"keys": list("ABCD"), **bounds, "step": 10, "monotone": "decreasing"}},
