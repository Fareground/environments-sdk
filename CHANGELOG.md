...
- **Audit 12.**
  ...
  - `visible()` writes an invisible character above U+FFFF as the surrogate pair JSON reads (`\udb40\udc41`), not
    `\U000e0041`, so a host request escaped with it is still valid JSON that decodes to what was typed.
  ...
  - Every field of every effect operation has one declared shape (`fg_env.effects.shapes`), read by the checker and
    ...
    rather than taken as true. Graph edges are read by one function in the checker and the built space (a list or
    object as an end used to crash `check`), and assigning an entity's built-in `id`, `name`, `type`, `alive` or `at`
    is a check error saying what changes each.
- **Audit 11.**
...
- **Audit 10.**
  ...
  - A view's `where` that reads a hidden value of an item before the term that picks the reader's own
...
- **Audit 9.**
  ...
  - An order book audits its accounting each round by default (`conserve: "round"`); `conserve: true` still audits
...
- **Audit 8.**
  ...
  - The `supply_chain` engine's default ordering policy (`anchor`) adjusts a quarter of the stock gap and a third as
    much of the supply-line gap, the weights fitted to people playing the beer game (Sterman 1989). Before, it
...
- **Audit 7.**
  ...
  - An assignment to a named entity (`$entity(a).vv = 1`) is checked like one to `$actor`: a misspelt property or a
    ...
    it, blaming `p.vv`.
  - A value a property refuses at run time (a fraction into an `int`, text into a number) is reported at the rule that
    wrote it, naming the property and where it is declared (`events[0].do[0]: \`$entity(a).n += 0.5\`: a's n
    (types.p.props.n) must be a whole number`); before, the path was `p.n`. An entity `count` that is not a number is
    a check error before anything runs.
