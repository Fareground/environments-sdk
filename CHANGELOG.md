...
- **Batches pay less for worker processes.** `experiment`, the analyses, `optimise`, `tournament` and `evaluate` keep
  ...
  whose runs are measured to finish before workers could start stays in this process, many short runs travel to a
  worker per round trip, and each worker parses a contract once. Results are unchanged whichever way a batch runs.
