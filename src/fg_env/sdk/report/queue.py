...
class QueueView:
    ...
    def plan_sentence(self, option: Option) -> str:
        spans = blocks(self.staff(option))
        if not spans:
            return ""
    ...
    def assumptions(self) -> List[str]:
        ...
        for name, channel in (self.config.get("channels") or {}).items():
            ...
            text = (f"{name.replace('_', ' ').capitalize()} arrive at random at the expected rate of each "
                    f"{unit_word(self.clock)}; service takes {_duration(service, self.unit)}")
            text += (f"; customers give up after {_duration(patience, self.unit)} of waiting" if isinstance(patience, Mapping)
                     else "; nobody gives up waiting")
            if isinstance(channel.get("callback"), Mapping):
                callback = channel["callback"]
                text += f"; a callback is offered when the wait would pass {callback.get('when', 0)} {plural(self.unit, 2)}"
            out.append(text + ".")
        return out
...
def _duration(spec: Mapping[str, Any], unit: str) -> str:
    dist = str(spec.get("dist") or "exponential")
    mean = spec.get("mean")
    shown = f"{mean:g} {plural(unit, 2)}" if isinstance(mean, (int, float)) else f"the value of {mean}"
    return f"{shown} on average ({dist})" if dist != "fixed" else shown


def _factor_name(contract: Any, factor: str, row: Mapping[str, Any]) -> str:
    spec = contract.patterns.get(factor) or {}
    name = str(spec.get("description") or factor.replace("_", " ")).rstrip(".")
