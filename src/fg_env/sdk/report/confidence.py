...
def interval(namer: Namer, measure: str, low: Optional[float], high: Optional[float]) -> str:
...
def lines(ev: Evidence, choice: Choice, sure: Confidence, namer: Namer) -> List[str]:
    ...
    if runner is None:
        ...
        out.append(f"{label(best, start=True)} and {label(runner)} are within noise on {namer.name(goal.measure)}"
                   + (f": {namer.change(goal.measure, margin.mean)} ({interval(namer, goal.measure, margin.low, margin.high)})"
