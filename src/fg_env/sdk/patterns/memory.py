...
class _Memory(PatternConfig):
    ...
    every: Optional[float] = Field(None, gt=0, description="Clock units per step of carry-over (default: one round).")
...
def _elapsed(ctx: Any, state: Optional[Dict[str, Any]]) -> float:
...
def _current(ctx: Any) -> Tuple[float, Optional[Dict[str, Any]], bool]:
...
def _saved(ctx: Any, x: Any, state: Optional[Dict[str, Any]], **values: Any) -> Dict[str, Any]:
...
class CarryoverConfig(_Memory):
    ...
    half_life: Optional[Number] = Field(None, description="Steps for a past input's effect to halve (instead of retain).")
...
def _stock(ctx: Any, x: float, before: Optional[Dict[str, Any]]) -> float:
...
def _carryover_commit(ctx: Any, x: Any, state: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
@kind("carryover", "memory", "memory", CarryoverConfig,
      ...
      words=lambda cfg: f"carry-over of {cfg.input} ({cfg.form}, retain {cfg.half_life and f'half-life {cfg.half_life}' or cfg.retain}"
...
def _pressure(ctx: Any, before: Optional[Dict[str, Any]]) -> float:
    return 0.0 if before is None else float(before["p"]) * _retain(ctx) ** _elapsed(ctx, before)


def _promotion_commit(ctx: Any, x: Any, state: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
def _reference(ctx: Any, x: float, before: Optional[Dict[str, Any]]) -> float:
    return x if before is None else float(before["r"])


def _reference_commit(ctx: Any, x: Any, state: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    value, before, committed = _current(ctx)
    if not committed:
        value, before = driver(ctx, x, "`input`"), state
...
class HabitConfig(_Memory):
    ...
    form: Literal["habit", "fatigue"] = Field("habit", description="habit: 1 + strength·S/(1 + S), growing with repetition | "
    ...
    half_life: Optional[Number] = Field(None, description="Steps for S to halve (instead of retain).")
...
def _exposure(ctx: Any, x: float, before: Optional[Dict[str, Any]]) -> float:
    previous = 0.0 if before is None else float(before["s"])
    return max(0.0, x) + _retain(ctx) ** _elapsed(ctx, before) * previous


def _habit_commit(ctx: Any, x: Any, state: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    value, before, committed = _current(ctx)
    if not committed:
        value, before = driver(ctx, x, "`input`"), state
...
@kind("habit", "population", "memory", HabitConfig,
      ...
      example={"kind": "habit", "form": "fatigue", "input": "$it.ads_seen", "keys": "viewer", "strength": 0.3, "half_life": 3},
