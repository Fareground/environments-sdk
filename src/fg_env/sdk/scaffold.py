...
_MARKET: Dict[str, Any] = {
    ...
    "actions": {
        ...
        "buy": {"by": "household", "description": "Buy loaves at today's price.",
                "params": {"qty": {"type": "int", "min": 1, "max": 3}},
                "when": {"expr": "$actor.cash >= $params.qty * $entity(mo).price", "why": "You cannot afford that many"},
...
_SIMULATION: Dict[str, Any] = {
    ...
    "clock": {"rounds": 30},
    "inputs": {"people": {"type": "int", "default": 50}},
    "types": {"person": {"props": {"wealth": 5}}},
    "population": [{"type": "person", "count": "$inputs.people"}],
    "events": [{"phase": "end", "each": "person", "where": "$it.wealth > 0",
