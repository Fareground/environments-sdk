...
Node = Union[Terminal, Chance, Decision]
...
def extract_tree(source: Union[Game, GameState], *, max_nodes: int = MAX_TREE_NODES) -> GameTree:
...
def _expand(state: GameState, counter: List[int], limit: int) -> Node:
    ...
    try:
        ...
        if unlisted:
            raise ValueError(f"seat {player} has calls that cannot be listed ({unlisted}), so the tree cannot hold them")
