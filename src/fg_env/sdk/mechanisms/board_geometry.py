...
class Geometry:
    ...
    def expand(self, dirs: object, where: str, forward: Optional[str] = None) -> Tuple[str, ...]:
        ...
        for item in items:
            ...
            if item in self.groups:
                ...
                raise GeometryError(f"{where}: '{item}' is not a direction on a {self.shape} board ({self._dir_help()})")
...
def _file_letters(count: int) -> List[str]:
    if count > 26:
        raise GeometryError(f"algebraic names have 26 column letters; this board has {count} columns — use \"coords\": \"rc\"")
...
def hex_board(radius: Optional[int] = None, rows: Optional[int] = None, cols: Optional[int] = None) -> Geometry:
