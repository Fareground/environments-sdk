...
``memory`` keeps, per agent, what it read and did each round, its own notes and host-written
...
class MemoryConfig(BaseModel):
    ...
    reflect_prompt: str = Field("Reflect on these memories: what matters most now, and what should you remember going forward? "
                                "Answer in two or three sentences.", description="What a reflection asks for.")
    reflect_fallback: Optional[Literal["skip"]] = Field(None, description="Without a writer: skip reflections (default: stop with an error).")
    ...
    for agent in agents_of(world, config.who):
        ...
        if "did" in config.capture:
            for lookup in lookups:
                ...
                items += [("did", f"You looked up {format_value(item['query'])} with {lookup}: {format_value(item['text'])}")
...
def _recall(runner: Any, effect: Dict[str, Any], vars: Dict[str, Any], where: str) -> None:
    ...
    header = f"Memories about {asked}:" if matched else f"Nothing in your memory matches {asked}; your strongest memories:"
...
class RecapConfig(BaseModel):
    ...
    prompt: str = Field("Summarise the story so far for participants who need to catch up: who did what, what "
                        "was decided, what is still open. Be faithful and brief.", description="What the recap asks for.")
...
def _expand_recap(name: str, config: RecapConfig, contract: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(source, Mapping):
        raise MechanismError(f"'{config.record}' is not a declared record", f"records: {', '.join(records) or 'none'}", "record")
    if name in records:
