...
def _expand_channeling(name: str, cfg: ChannelConfig, contract: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    for action in cfg.actions:
        hooks[action]["do"] = [{"channel": action}]
    common.hook_actions(contract, hooks, "actions")
...
def _check_channel(checker: Any, effect: Dict[str, Any], path: str) -> List[Tuple[str, str, Optional[str]]]:
    ...
    action = effect.get("channel")
    return [] if action in index else [(f"{path}.channel", f"'{action}' is not a channeled action", common.suggest(str(action), index))]


@effect_op("channel", keys=(), literal=("channel",), check=_check_channel,
           example='{"channel": "meteor"}  (start channeling with this action\'s $actor and $params; added automatically)')
def _channel_op(runner: Any, effect: Dict[str, Any], vars: Dict[str, Any], where: str) -> None:
    world = runner.world
    action = effect["channel"]
    ...
    if not isinstance(actor, Entity):
        raise RunError("`channel` runs inside an action ($actor)", where)
