"""Turn order beyond seats: initiative, rotating first player, snake order, skips and extra turns.

.. code-block:: json

    "initiative": {"kind": "turn_order", "among": "unit", "by": "$it.speed", "skip": "$it.hp <= 0",
...
right after it, which wakes agents granted an extra turn with ``{"extra_turn": entity, "order": name}``
until none is left (one action per extra turn).
"""
...
__all__ = ["TurnOrderConfig", "ordered"]

KIND = "turn_order"


class TurnOrderConfig(Config):
    """Who acts in which order."""

    among: str = Field(..., description="Agent type whose order this is (subtypes included).")
    ...
    extra_turns: bool = Field(False, description="Also declare <name>_extra after the stage for extra turns granted with the extra_turn effect.")
    max_extra: int = Field(3, ge=1, description="Most extra turns one agent can chain in a round.")
    view: bool = Field(True, description="Show agents this round's order.")


@mechanism(KIND, TurnOrderConfig,
           ...
           "this round's order.",
           example={"kind": KIND, "among": "player", "by": "$it.speed", "skip": "$it.folded", "rotate": True,
                    "stage": {"actions": ["bet", "fold"], "turns": "sequential"}})
def _expand(name: str, cfg: TurnOrderConfig, contract: Mapping[str, Any]) -> Dict[str, Any]:
    common.types_in(contract, cfg.among, "among")
    if not _agent(contract, cfg.among):
        raise MechanismError(f"'{cfg.among}' is not an agent type", f"set types.{cfg.among}.agent: true", "among")
    ...
    if cfg.view:
        fragment["views"] = {name: {"for": cfg.among, "title": "Turn order",
                                    "show": f"{{$join($map($turn_order('{name}'), $it.name), ', ')}}"}}
    if cfg.stage is None:
        return fragment
    stage = copy.deepcopy(cfg.stage)
    if "order" in stage:
        raise MechanismError("turn_order sets the stage's `order`", "remove `order`", "stage.order")
    stage.setdefault("name", name)
    stage["order"] = f"$turn_rank($it, '{name}')"
    sits_out = f"not ({cfg.skip})" if cfg.skip else None
    stage["who"] = _both(_both(f"$is($it, {cfg.among})", sits_out), stage.get("who"))
    stages = [_validated(stage, "stage")]
    if cfg.extra_turns:
        counter = f"{name}_extra"
        fragment["types"] = {cfg.among: {"props": {counter: {"type": "int", "default": 0, "min": 0, "max": cfg.max_extra,
        ...
        extra = {**copy.deepcopy(cfg.stage), "name": counter, "order": stage["order"], "turns": "sequential",
                 ...
                 "until": f"$count({cfg.among}, {waiting}) == 0", "passes": cfg.max_extra, "quiet": "wake",
                 ...
                 "on_exit": [*(cfg.stage.get("on_exit") or []),
                             {"each": cfg.among, "where": f"$it.{counter} > 0", "do": [f"$it.{counter} = 0"]}]}
...
def ordered(world: Any, name: str, where: str) -> List[Entity]:
    """This round's order for the turn_order mechanism ``name`` (skipped agents left out)."""
    cfg = common.config(world, name, KIND, TurnOrderConfig, where)
    ...
    kinds = set(world.contract.subtypes(cfg.among))
...
def _check_extra(checker: Any, effect: Dict[str, Any], path: str) -> List[Tuple[str, str, Optional[str]]]:
    name = effect.get("order")
    raw = checker.c.mechanisms.get(name)
    if not isinstance(raw, Mapping) or raw.get("kind") != KIND:
        return [(f"{path}.order", f"'{name}' is not a declared {KIND} mechanism", None)]
    if not raw.get("extra_turns"):
        return [(f"{path}.order", f"turn order '{name}' does not allow extra turns", "set extra_turns: true")]
    return []


@effect_op("extra_turn", keys=("order",), required=("order",), literal=("order",), check=_check_extra,
           example='{"extra_turn": "$actor", "order": "initiative"}  (the entity takes another turn this round)')
def _extra_op(runner: Any, effect: Dict[str, Any], vars: Dict[str, Any], where: str) -> None:
    ...
    counter = f"{name}_extra"
    for entity in common.entities_of(world, runner.eval(effect["extra_turn"], vars), where):
