...
from .econ_base import (DEMAND, REPLENISHMENT, compiles, declared_use, register_config, require_currency, require_types,
                        type_list)
...
class LeadTimeRef(BaseModel):
    ...
    pattern: str = Field(..., description="A noise pattern (normal, lognormal, uniform or laplace), drawn once per order.")
    key: Optional[str] = Field(None, description="Its key as an expression over $it, for a keyed pattern (default: the item).")
    scale: Union[float, str] = Field(1.0, description="Multiplies the draw (expression over $it): \"$it.lead_weeks\" with a "
...
class ReplenishmentConfig(BaseModel):
    ...
    demand: str = Field(..., description="The economy.demand mechanism whose items and stock this keeps (declared before it).")
    policy: str = Field("s_S", description="s_S: when the position is at or below reorder_point, order up to order_up_to | "
                                           "s_Q: order order_qty (repeated until above the reorder point) | order_up_to: "
                                           "order up to order_up_to every review (base stock; periodic with review_every) | "
                                           "service: order up to $target, from service_level | custom: order what `decide` "
                                           "gives | manual: only orders placed by agents or effects. Or an expression over "
                                           "$inputs and $it giving one (for arms).")
    reorder_point: Union[float, str, None] = Field(None, description=f"s, as an expression over $it and {_LOCALS}.")
    order_up_to: Union[float, str, None] = Field(None, description="S, as an expression over the same locals.")
    order_qty: Union[float, str, None] = Field(None, description="Q, as an expression over the same locals.")
    decide: Optional[str] = Field(None, description="custom: units to order now, as an expression over the same locals.")
    review_every: Union[int, str] = Field(1, description="Rounds between reviews (number or expression over $it).")
    ...
    forecast_sd: Union[float, str, None] = Field(None, description="Standard deviation of demand per round (default: from "
                                                                   ...
                                                                   "or √forecast).")
    lead_time: Union[float, str, LeadTimeRef] = Field(1.0, description="Clock units from order to arrival: a number or an "
                                                                      "expression over $it (known), or {pattern, key, scale} "
                                                                      "drawn per order. An order arrives at the start of "
                                                                      "the round that many units later (at least the next).")
    case_pack: Union[int, str] = Field(1, description="Orders come in multiples of this (expression over $it).")
    min_order: Union[int, str] = Field(0, description="Smallest order (expression over $it).")
    max_order: Union[int, str, None] = Field(None, description="Largest order (expression over $it).")
    capacity: Union[float, str, None] = Field(None, description="Most units on hand plus on order (expression over $it).")
    budget: Union[float, str, None] = Field(None, description="Money for orders each round across all items (expression); "
                                                              "the most urgent items are ordered first.")
    unit_cost: Union[float, str, None] = Field(None, description="Purchase price per unit (default: the demand's cost).")
    holding_cost: Union[float, str] = Field(0.0, description="Cost per unit on hand per round (expression over $it).")
    order_cost: Union[float, str] = Field(0.0, description="Cost per order placed (expression over $it).")
    stockout_cost: Union[float, str] = Field(0.0, description="Cost per unit of lost demand (expression over $it).")
    backorder_cost: Union[float, str] = Field(0.0, description="Cost per unit on backorder per round (expression over $it).")
    account: Optional[str] = Field(None, description="Entity id paying for orders from its ledger balance (default: the "
                                                     "demand's account); orders it cannot afford are cut.")
    currency: Optional[str] = Field(None, description="The ledger currency of `account` (default: the demand's).")
    who: Union[str, List[str], None] = Field(None, description="Agent type(s) that may order with the tool <name>_order.")
    record: Union[bool, str] = Field(False, description="Post every order to the record <name>_orders when it arrives: time, "
...
_DOC = ("Inventory policies for a demand mechanism's items: orders travel in a per-item pipeline for a lead time (fixed, "
        "or drawn per order from a noise pattern fitted from purchase orders) and arrive at the start of a round; at the "
        "end of every round, after the sales, each item's position (on hand + on order − backorders) is reviewed and "
        "the `policy` orders: s_S, s_Q, order_up_to (base stock, periodic with review_every), service (order up to the "
        "forecast over lead time + review plus z·σ safety stock for the service_level, σ from the forecast's error and "
        "the lead time's spread), custom (`decide`) or manual (agents' <name>_order tool and the `order` action). Orders "
        "respect case_pack, min_order, max_order, capacity and a round's budget, and are paid from a ledger account. "
        "Holding, ordering, stockout and backorder costs accrue per item; outputs <name>_orders, _purchases, "
        "_holding_cost, _order_cost, _stockout_cost, _backorder_cost, _total_cost, _profit (the demand's margin less "
        "these costs) and _average_stock_value.")
...
def _expand_replenishment(name: str, config: ReplenishmentConfig, contract: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if name in order and order.index(config.demand) > order.index(name):
        raise MechanismError(f"'{config.demand}' is declared after '{name}'",
                             f"declare '{config.demand}' first, so a round's sales are counted before reordering", "demand")
    ...
    for field in ("policy", "reorder_point", "order_up_to", "order_qty", "decide", "review_every", "service_level",
                  "forecast_sd", "case_pack", "min_order", "max_order", "capacity", "budget", "unit_cost", "holding_cost",
                  "order_cost", "stockout_cost", "backorder_cost", "record"):
    ...
    if (account is None) != (currency is None):
        raise MechanismError("`account` and `currency` go together", "give both to pay for orders from a ledger, or neither", "account")
    if currency is not None:
        require_currency(contract, currency)
    items = demand.items
    fragment: Dict[str, Any] = {
        "types": {items: {"props": _item_props(name)}},
        "world": {f"{name}_spent": {"type": "map", "default": {}, "description": "Money spent on orders this round: {round, amount}."}},
    ...
    if config.who is not None:
        ...
        fragment["actions"] = {f"{name}_order": {
            "by": agents, "description": f"Order stock of one {items} from its supplier; it arrives after the lead time.",
...
def _check_policy(config: ReplenishmentConfig) -> None:
    ...
    if policy not in POLICIES:
        raise MechanismError(f"'{policy}' is not a policy", f"policies: {', '.join(POLICIES)} (or an expression giving one)", "policy")
    missing = [field for field in NEEDS[policy] if getattr(config, field) is None]
    if missing:
        raise MechanismError(f"policy {policy} needs `{missing[0]}`", f"policy {policy} reads {', '.join(NEEDS[policy])}", missing[0])
...
def _check_lead_time(config: ReplenishmentConfig, contract: Mapping[str, Any]) -> None:
    ...
    if not isinstance(spec, Mapping):
        raise MechanismError(f"'{lead.pattern}' is not a declared pattern", "declare a noise pattern for the lead time", "lead_time.pattern")
    if spec.get("kind") != "noise" or spec.get("dist", "normal") not in _NOISE_DISTS:
        raise MechanismError(f"'{lead.pattern}' is not a noise pattern",
                             "a lead time is drawn per order from {\"kind\": \"noise\", \"dist\": \"lognormal\", ...}", "lead_time.pattern")
...
def _item_props(name: str) -> Dict[str, Any]:
    ...
    return {
        f"{name}_pipeline": {"type": "list", "default": [], "description": "Orders on the way: [round due, units, round placed, "
                                                                           "lead time, lead-time draw]."},
...
def _outputs(name: str, demand_name: str, demand: DemandConfig) -> Dict[str, Any]:
    ...
    out[f"{name}_average_stock_value"] = {"expr": f"$round($avg($series.{name}_stock_value, $it), 2) if "
                                                  f"$len($series.{name}_stock_value) > 0 else 0", "type": "number", "format": "money"}
    if demand.group is not None:
        out[f"{name}_total_cost_by_group"] = {"expr": f"$replenishment_totals('{name}', 'total_cost', 'group')", "type": "map"}
    return out
