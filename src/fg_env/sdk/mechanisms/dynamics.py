...
@function("triangular(low, mode, high)", "Triangular-distributed number between low and high, most likely at mode.",
          min_args=3, max_args=3)
def _triangular(call: Call) -> float:
    low, mode, high = call.number(0), call.number(1), call.number(2)
    if not low <= mode <= high:
        raise ExprError("$triangular needs low ≤ mode ≤ high", call.source)
