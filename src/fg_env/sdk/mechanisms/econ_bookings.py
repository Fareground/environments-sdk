...
class ResourceSpec(BaseModel):
    ...
    provider: Optional[str] = Field(None, description="Entity id paid for bookings (needed when price > 0).")
    name: str = ""
...
def _expand_bookings(name: str, config: BookingsConfig, contract: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if config.order == "priority" and not config.priority:
        raise MechanismError("priority order needs a `priority` expression", "e.g. \"priority\": \"$it.loyalty\"", "priority")
    ...
    fragment: Dict[str, Any] = {
        "types": {
            ...
            booking: {"description": "A booking or a place in line.", "props": {
                ...
                "status": {"type": "enum", "values": ["booked", "waiting", "served", "cancelled", "abandoned", "turned_away"],
                           "default": "waiting"},
    ...
    params["party"] = {"type": "int", "min": 1, "max": guarded("$params.resource.max_party", "resource"), "default": 1,
                       "description": "Places."}
    do: Dict[str, Any] = {"book": name, "guest": "$actor", "resource": "$params.resource", "party": "$params.party"}
    if config.mode == "slots":
...
def _bookings(world: Any, name: str, resource: Optional[str] = None, slot: Optional[int] = None,
...
def _promote(world: Any, name: str, config: BookingsConfig, resource: Any, slot: int, where: str) -> None:
    ...
    for booking in _in_order(config, _bookings(world, name, resource.id, slot, "waiting")):
        ...
        emit_to(world, f"{name}_promoted", f"A place opened: you are booked at {resource.name} for round {slot}"
                + (f" (paid {money(paid)} {config.currency})." if paid else "."), [guest.id], why=f"You got a place at {resource.name}.")


@function("booking_text(agent, bookings)", "The agent's latest booking of a bookings mechanism, in words.", min_args=2, max_args=2)
def _booking_text(call: Call) -> str:
    ...
    if p["status"] == "booked":
        return f"Booked {p['party']} place(s) at {where}{when}" + (f", paid {money(p['paid'])} {config.currency}." if p["paid"] else ".")
    if p["status"] == "waiting":
...
@function("places_text(resource, bookings)", "Free places of a resource in the coming rounds (slots) or the line length (queue).",
...
@effect_op("book", keys=("guest", "resource", "ahead", "party"), required=("guest", "resource"), literal=("book",),
           example='{"book": "dining", "guest": "$actor", "resource": "$params.resource", "ahead": 2, "party": 4}  '
...
def _book(runner: Any, effect: Dict[str, Any], vars: Dict[str, Any], where: str) -> None:
    ...
    if any(props(b)["guest"] == guest.id and props(b)["status"] in LIVE for b in _bookings(world, name, resource.id, slot)):
        raise Abort(f"You already have a booking at {resource.name}" + (f" for round {slot}." if slot else "."))
    values: Dict[str, Any] = {"guest": guest.id, "resource": resource.id, "slot": slot, "party": party, "joined": world.round}
    if config.priority:
...
def _number(world: Any, value: Union[int, str], guest: Any, where: str) -> float:
...
def _cancel_booking(runner: Any, effect: Dict[str, Any], vars: Dict[str, Any], where: str) -> None:
    ...
    if refund > 0 and config.currency is not None:
        provider = entity_of(world, props(resource)["provider"], where, "the resource's provider")
        move_money(world, config.currency, provider, entity_of(world, p["guest"], where), refund, where, use_credit=False)
...
def _bookings_tick(runner: Any, effect: Dict[str, Any], vars: Dict[str, Any], where: str) -> None:
    ...
    for resource in world.entities_of(f"{name}_resource"):
        ...
        if config.mode == "slots":
            ...
            for booking in [b for b in _bookings(world, name, resource.id, status="waiting") if int(props(b)["slot"]) <= world.round]:
                world.set_prop(booking, "status", "turned_away")
                _stat(world, name, "turned_away", 1)
                emit_to(world, f"{name}_turned_away", f"No place opened at {resource.name}; your waitlist spot has lapsed.",
                        [props(booking)["guest"]])
            continue
        left = capacity
        for booking in _in_order(config, _bookings(world, name, resource.id, 0, "waiting")):
            ...
            try:
                ...
                world.set_prop(booking, "status", "turned_away")
                _stat(world, name, "turned_away", 1)
                emit_to(world, f"{name}_turned_away", f"You reached the front at {resource.name} but could not pay: {exc.reason}", [guest.id])
                continue
            world.set_prop(booking, "paid", paid)
            _stat(world, name, "revenue", paid)
            _serve(world, name, booking, resource)
            left -= party
            emit_to(world, f"{name}_served", f"You were served at {resource.name}" + (f" and paid {money(paid)} {config.currency}." if paid else "."),
                    [guest.id])
