...
class Ledger:
    ...
    def _scored(self, seating: Mapping[str, str], scores: Mapping[str, float], results: Mapping[str, str]) -> List[Pairing]:
    ...
    def standings(self, kind: str) -> List[Dict[str, Any]]:
        ...
        for rank, name in enumerate(names, 1):
            rows.append({"rank": rank, "entrant": name, "rating": self.rating(name, kind), "elo": self.elo[name].to_dict(),
    ...
    def evaluation(self) -> Dict[str, Any]:
        ...
        return {"margins": {a: {b: payoff[i][j] for j, b in enumerate(self.entrants)} for i, a in enumerate(self.entrants)},
