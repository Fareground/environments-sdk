...
class ArmResult:
    ...
    #: Inputs of this arm the experiment's own inputs replaced, in plain words (see :mod:`fg_env.arm_inputs`).
...
def worker_pool(workers: int, participants: Any = None, hosts: Any = None) -> Iterator[Optional[pools.Workers]]:
    """Worker processes shared by many :func:`run_jobs` calls: this process's kept pool (:mod:`fg_env.workers`),
...
def run_jobs(source: ContractLike, jobs: Sequence[Job], *, participants: Any = None,
    ...
    when the batch is too short to pay for workers: :mod:`fg_env.workers`); callables run in threads.
    ...
    (every run has the whole budget: :mod:`fg_env.budget`); ``exposures=True`` records what agents saw in every
