"""The start page for human and agent authors: ``guide('authoring')``. ``guide()`` is the map of every other part, so
...
2. **State** — `world` holds global props (`$world.fish`); `types` declare each kind's props (a bare value is the
   default: `"cash": 100` holds any number, 12.5 too — `"type": "int"` for whole numbers only — or
   `{type, default, min, max, values, private}`); `entities` are named
