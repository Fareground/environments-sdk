"""The core of the contract check: the issues it reports, and checking one expression, template or value.

Each part of the check (:mod:`.effects`, :mod:`.world`, :mod:`.actions`, :mod:`.privacy`, :mod:`.rules`) builds on
:class:`Checker`; ``checks/__init__.py`` puts them together and runs them in order.
"""
...
from .roots import ENTITY_FIELDS, Types
...
class Checker:
    """What every part of the check shares: the contract, the issues found, and checking one expression, template,
    value or setting where it is written."""
    ...
    def _count(self, value: Any, path: str) -> None:
    ...
    def _refs(self, compiled: Any, path: str, roots: set[str], types: Types,
        ...
        for function, kind, word, position in item_words(compiled.source) if compiled.calls else ():
            per_item = function in FUNCTIONS and position in FUNCTIONS[function].lazy  # evaluated for each item
            if per_item and kind in self.c.types and word in self.c.props_of(kind):
