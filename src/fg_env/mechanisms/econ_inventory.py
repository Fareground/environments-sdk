...
class ItemSpec(BaseModel):
    ...
    unique: bool = Field(False,
                         description="Each unit is its own entity (type named after the item) with an owner and props.")
    ...
    description: str = ""
    shelf_life: int | None = Field(None, ge=1, description="Rounds until a unit spoils (oldest units first).")
    decay: float | None = Field(None, ge=0, le=1, description="Chance each unit is lost every round (seeded).")
...
class InventoryConfig(BaseModel):
    ...
    who: str | list[str] = Field(..., description="Type(s) that hold goods (subtypes included).")
    items: dict[str, ItemSpec] = Field(..., min_length=1,
                                       description="{item: {unique, value, size, unit, shelf_life, decay, props, "
                                                   "consumable, on_consume}}.")
    prop: str | None = Field(None,
                             description="Holder property with the stackable goods {item: qty}; default the "
                                         "mechanism's name.")
    capacity: float | str | None = Field(None,
                                         description="Space each holder has (number or expression); unlimited when "
                                                     "omitted.")
    start: dict[str, int | str] = Field(
...
def baseline(contract: Mapping[str, Any], holders: list[str], assets_: list[str], loose: str | None = None) -> str:
...
def _start_default(start: dict[str, int | str], stackable: list[str]) -> Any:
...
def _list_every_item(contract: Mapping[str, Any], holders: list[str], prop: str, stackable: list[str]) -> None:
    ...
    for entry in entries:
        goods = (entry.get("props", {}).get(prop) if isinstance(entry, dict) and isinstance(entry.get("props"), dict)
                 else None)
...
def _expand_inventory(name: str, config: InventoryConfig, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not valid_name(prop):
        raise MechanismError(f"prop '{prop}' is not a property name",
                             "use letters, digits and _ (not a word expressions use, like in or not)", "prop")
    taken = _other_names(contract, name)
    for item, spec in config.items.items():
        if not valid_name(item):
            raise MechanismError(f"item '{item}' is not a valid name",
                                 "use letters, digits and _ (not a word expressions use, like in or not)",
                                 f"items.{item}")
        if item in taken:
            raise MechanismError(f"'{item}' is already declared by '{taken[item]}'",
                                 "give every item and currency its own name", f"items.{item}")
        if spec.unique and (spec.consumable or spec.on_consume or spec.shelf_life is None and spec.decay is not None):
            raise MechanismError(f"unique item '{item}' cannot be consumable or decay",
                                 "use a stackable item, or `use` it in your own action", f"items.{item}")
        if spec.props and not spec.unique:
            raise MechanismError(f"item '{item}' has props but is not unique", "set \"unique\": true",
                                 f"items.{item}.props")
    ...
    if ("drop" in config.actions or "pickup" in config.actions) and not contract.get("space"):
        raise MechanismError("drop and pickup need a declared space", "declare `space`, or remove them from actions",
                             "actions")
    for type_name, wants in config.needs.items():
        require_types(contract, [type_name], f"needs.{type_name}")
        for item in wants:
            if item not in config.items or config.items[item].unique:
                raise MechanismError(f"needs: '{item}' is not a stackable item of this inventory", None,
                                     f"needs.{type_name}.{item}")
    ...
    holder_props: dict[str, Any] = {prop: {"type": "map", "default": _start_default(config.start, stackable),
                                           "description": f"Goods held ({name}): {{item: quantity}}, every stackable "
                                                          "item listed."}}
    ...
    world: dict[str, Any] = {
        ...
        f"{name}_flows": {"type": "map", "default": {},
                          "description": "Goods made (+) and used up (−) by each named source and sink."},
    }
    if "drop" in config.actions or "pickup" in config.actions:
        world[f"{name}_ground"] = {"type": "map", "default": {},
                                   "description": "Goods lying at each place: {place: {item: qty}}."}
    ...
    if agents:
        ...
        if f"{name}_ground" in world:
            fragment["views"][f"{name}_here"] = {"for": agents, "title": "Goods lying here", "bullet": False,
                                                 "when": f"$actor.at != null and $len($keys($ground_items('{name}', "
                                                         "$actor.at))) > 0",
...
def _actions(name: str, config: InventoryConfig, contract: Mapping[str, Any], holders: list[str],
    ...
    if "give" in config.actions:
        ...
        actions[f"{name}_give"] = {
            ...
            "params": {"to": to,
                       "item": {"type": "enum", "values": owned,
                                "description": "Item you hold (a name, or the id of a unique item)."},
                       "qty": qty},
            "do": [{"economy": name, "action": "give", "item": "$params.item", "from": "$actor", "to": target,
                    "qty": "$params.qty"}],
    ...
    if "consume" in config.actions and consumables:
        ...
        effects: list[Any] = [{"economy": name, "action": "use", "item": "$params.item", "from": "$actor",
                               "qty": "$params.qty", "sink": "consumed"},
                              "$qty = $params.qty"]
    ...
    if "drop" in config.actions and stackable:
        ...
        actions[f"{name}_drop"] = {
            ...
            "when": [{"expr": f"$actor.at != null and $len({choices}) > 0",
                      "why": "You hold nothing you can put down here."}],
    ...
    if "pickup" in config.actions and stackable:
        ...
        actions[f"{name}_pickup"] = {
            ...
            "params": {"item": {"type": "enum", "values": f"$keys({here})", "description": "What to pick up."},
                       "qty": {"type": "int", "min": 1, "max": guarded(f"$get({here}, $params.item, 0)", "item"),
                               "default": 1, "description": "How many."}},
...
@family_action("economy", ("inventory",), "tick", internal=True,
               example='{"economy": "goods", "action": "tick"}  (spoil goods past their shelf life and apply decay, '
                       'now)')
