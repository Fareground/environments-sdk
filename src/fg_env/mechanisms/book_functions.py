...
@function("book(name)", "Top of an order book: {last, bid, ask, mid, spread, bid_qty, ask_qty, ref, halted, halt_until, "
          "open, high, low, round_volume, round_trades, volume, vwap, trades, fees, halts, orders, flow, liquidations, bar, "
          "bar_rounds, tick, lot, maker_fee_bps, taker_fee_bps, short_limit, halt_pct, band_low, band_high}; flow is the last "
          "round's aggressive {buy, sell} quantity by trader kind; bar is the bar in progress {bar, open, high, low, close, "
          "volume, vwap, trades, halted, flow, ends}.", min_args=1, max_args=1)
...
_ACTIONS: Dict[str, Tuple[Tuple[str, ...], Tuple[str, ...], bool, str, str]] = {
    ...
    "close": ((), (), False, "", "close the round (once a round; the book's own end event runs it after yours, so run it "
                                 "first in an end event that reads the closed round or bar): breaker check, record the bar"),
}
