...
class SourceSpec(BaseModel):
    ...
    mode: Literal["add", "top_up", "reset"] = Field("add",
                                                    description="add | top_up (up to amount) | reset (unspent money "
                                                                "expires, then amount).")
...
class TaxSpec(BaseModel):
    ...
    rate: float | str = Field(...,
                              description="Share of the payment (number or expression over $payer, $payee, $amount).")
    on: Literal["payee", "payer"] = Field("payee",
                                          description="payee: withheld from what is received; payer: added on top.")
    to: str | None = Field(None,
                           description="Entity id collecting it; omitted = the money leaves the economy (a sink).")
    description: str = ""
...
class LoanSpec(BaseModel):
    ...
    lenders: str | list[str] = Field(..., description="Type(s) that lend at a posted rate.")
    borrowers: str | list[str] = Field(..., description="Type(s) that may borrow.")
    ...
    max_amount: float | str = Field(1000.0, description="Largest loan (number or expression over $actor).")
...
class LedgerConfig(BaseModel):
    ...
    who: str | list[str] = Field(..., description="Type(s) holding money (subtypes included).")
    currencies: dict[str, CurrencySpec] = Field(..., min_length=1,
                                                description="{currency: {start, credit, unit, value}}; each is a "
                                                            "holder property ($actor.cash).")
    sources: dict[str, SourceSpec] = Field({},
                                           description="Scheduled money creation: {name: {to, amount, every, mode}}.")
    taxes: dict[str, TaxSpec] = Field({}, description="Levies payments can name: {name: {rate, on, to}}.")
    loans: LoanSpec | None = Field(None, description="Loans at posted rates with interest, due dates and default.")
...
def _currency(config: LedgerConfig, given: str | None, field: str) -> str:
    if given is None:
        if len(config.currencies) != 1:
            raise MechanismError("this ledger has several currencies; name one",
                                 f"currencies: {', '.join(config.currencies)}", field)
        return next(iter(config.currencies))
    if given not in config.currencies:
        raise MechanismError(f"'{given}' is not a currency of this ledger",
                             f"currencies: {', '.join(config.currencies)}", field)
    return given
...
def _expand_ledger(name: str, config: LedgerConfig, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for currency, spec in config.currencies.items():
        if not valid_name(currency):
            raise MechanismError(f"currency '{currency}' is not a valid name",
    ...
    fragment: dict[str, Any] = {
        ...
        "world": {f"{name}_supply": {"type": "map", "default": f"$money_held('{name}')",
                  ...
                  f"{name}_flows": {"type": "map", "default": {},
                                    "description": "Money created (+) and destroyed (−) by each named source and "
                                                   "sink."}},
...
def _source_event(name: str, config: LedgerConfig, source: str, spec: SourceSpec,
    ...
    if spec.mode == "add":
        ...
        effects = [{"if": f"$it.{currency} > 0",
                    "then": [{"economy": name, "action": "burn", "currency": currency, "from": "$it",
...
def _pay_action(name: str, config: LedgerConfig, holders: list[str], agents: list[str]) -> dict[str, Any]:
    ...
    if len(config.currencies) == 1:
        ...
        params["amount"] = {"type": "number", "min": 0.01, "max": _money_left(currency, spec),
                            "description": f"Amount of {currency}."}
        which: str = currency
    else:
...
def _loans(name: str, config: LedgerConfig, loans: LoanSpec, contract: Mapping[str, Any],
    ...
    fragment["types"][loan_type] = {"description": "A loan: owed grows by `rate` each round until repaid or due.",
                                    "props": {
    ...
    lender, lender_ref = choice_param(lenders, f"$it.{name}_lending and $it.id != $actor.id",
                                      "Lender (see their posted rate).")
    ...
    if borrowing_agents:
        fragment["actions"][f"{name}_borrow"] = {
            "by": borrowing_agents,
            "description": f"Borrow {currency} at the lender's posted rate per round; repay before the due round or "
                           "default.",
            "params": {"lender": lender,
                       "amount": {"type": "number", "min": 1, "max": loans.max_amount,
                                  "description": f"Amount of {currency}."},
        ...
        fragment["actions"][f"{name}_repay"] = {
            ...
            "params": {"loan": {"type": "entity", "of": loan_type,
                                "where": "$it.borrower == $actor.id and $it.status == active",
    ...
    if lending_agents:
        fragment["actions"][f"{name}_set_rate"] = {
            "by": lending_agents,
            "description": "Post the interest rate per round for new loans, and whether you lend at all.",
    ...
    if parties:
        views[f"{name}_loans"] = {"for": parties, "title": "Your loans", "of": loan_type,
                                  "where": "$it.status == active and ($it.borrower == $actor.id or $it.lender == "
                                           "$actor.id)",
                                  "show": "[{id}] {$'you owe' if $it.borrower == $actor.id else 'owed to you'} "
                                          "{owed|money} by round {due} ({rate|pct}/round)"}
    if borrowing_agents:
        views[f"{name}_lenders"] = {"for": borrowing_agents, "title": "Lenders", "look": True,
                                    "of": lenders[0] if len(lenders) == 1 else None, "where": f"$it.{name}_lending",
                                    "show": f"[{{id}}] {{name}}: {{{name}_rate|pct}} per round"}
        if len(lenders) != 1:
...
@family_action("economy", ("ledger",), "lend", keys=("from", "to", "amount", "rate", "term"),
               required=("from", "to", "amount", "term"), check=_check_loans,
               example='{"economy": "money", "action": "lend", "from": "$params.bank", "to": "$actor", "amount": '
                       '100, "term": 6}  (a loan: pays the principal now; rate defaults to the lender\'s posted rate)')
def _lend(runner: Any, effect: dict[str, Any], vars: dict[str, Any], where: str) -> None:
    ...
    rate = (runner.eval(effect["rate"], vars) if "rate" in effect
            else props(lender).get(f"{name}_rate", config.loans.rate_min))
...
@family_action("economy", ("ledger",), "repay", keys=("loan", "amount"), required=("loan", "amount"),
               check=_check_loans,
...
@family_action("economy", ("ledger",), "tick", internal=True,
               example='{"economy": "money", "action": "tick"}  (accrue loan interest, collect loans that are due, '
                       'default unpaid ones)')
def _ledger_tick(runner: Any, effect: dict[str, Any], vars: dict[str, Any], where: str) -> None:
    ...
    for loan in [e for e in world.entities_of(f"{name}_loan") if props(e).get("status") == "active"]:
        ...
        if props(loan)["status"] == "repaid":
            emit_to(world, f"{name}_loan", f"{borrower.name}'s loan from {lender.name} is repaid.",
        ...
        if world.round >= p["due"] + config.loans.grace:
            ...
            if config.loans.on_default:
                run_hook(runner, f"{name}_on_default",
                         {"loan": loan, "lender": lender, "borrower": borrower, "unpaid": unpaid},
