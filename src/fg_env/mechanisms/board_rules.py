...
Dirs = str | list[str]
Zone = str | list[str] | dict[str, str | list[str]]
...
class MoveSpec(_Strict):
    ...
    step: Dirs | None = Field(None,
                              description="Move `distance` cells in these directions through empty cells "
                                          "(\"adjacent\": to any neighbouring cell, e.g. on graphs).")
    slide: Dirs | None = Field(None, description="Move any number of cells along these directions until blocked.")
    leap: list[Any] | None = Field(None,
                                   description="Jump to an offset [forward, right] (or a list of them), ignoring what "
                                               "is between; every mirror and rotation unless `exact`.")
    jump: Dirs | None = Field(None,
                              description="Hop over the adjacent piece in these directions onto the empty cell beyond.")
    distance: int = Field(1, ge=1, description="step: exact number of cells (cells passed over must be empty).")
    max: int | None = Field(None, ge=1, description="slide: at most this many cells.")
    only: Literal["move", "capture"] | None = Field(None,
                                                    description="move: only onto empty cells; capture: only onto "
                                                                "enemies.")
    first: bool = Field(False, description="Only for a piece that has not moved yet.")
    from_: Zone | None = Field(None, alias="from", description="Only from cells in this zone.")
    within: Zone | None = Field(None, description="Only onto cells in this zone.")
    over: Literal["enemy", "own", "any"] = Field("enemy", description="jump: which pieces may be jumped.")
    capture: bool | None = Field(None, description="jump: remove the jumped piece (default: when jumping enemies).")
    chain: bool = Field(False,
                        description="After a capture with this pattern the same piece keeps capturing while it can "
                                    "(multi-jump).")
    passable: bool = Field(False,
                           description="A multi-cell step that an en_passant pattern can capture on the cell passed "
                                       "over.")
    en_passant: bool = Field(False,
                             description="May also capture a piece that just made a `passable` step over the target "
                                         "cell.")
...
class PromoteSpec(_Strict):
    ...
    to: list[str] = Field(..., min_length=1,
                          description="Piece kinds it may become; several = the mover chooses (e7-e8=Q).")
...
class PieceSpec(_Strict):
    ...
    name: str | None = Field(None, description="Readable name (default: the kind key).")
    symbol: str | dict[str, str] | None = Field(None, description="One-character board symbol, or {side: symbol}.")
...
class SideSpec(_Strict):
    ...
    id: str
    name: str | None = None
    forward: str | None = Field(None,
                                description="Direction this side's pieces advance (grid default: n for the first side, "
                                            "s for others).")
    mark: str | None = Field(None,
                             description="Board symbol when there is a single piece kind (default: first letter of the "
                                         "id).")
...
class PlaceSpec(_Strict):
    ...
    kinds: list[str] | None = Field(None, description="Kinds that can be placed (default: every kind).")
    from_: Literal["supply", "hand"] = Field("supply", alias="from",
                                             description="supply: unlimited; hand: counts in `hand`.")
    where: Zone | None = Field(None, description="Only on cells in this zone.")
    gravity: str | None = Field(None,
                                description="A placed piece falls this way to the last empty cell (\"s\" for Connect "
                                            "Four).")
...
class CaptureSpec(_Strict):
    ...
    rule: Literal["custodial", "flip", "enclose"] = Field(...,
                                                          description="custodial: sandwich an enemy between the moved "
                                                                      "piece and a friend; flip: turn over enemy "
                                                                      "lines closed by a friend (Othello); enclose: "
                                                                      "remove groups without liberties (Go).")
    dirs: Dirs | None = Field(None, description="custodial default orthogonal; flip default all.")
    hostile: Zone | None = Field(None,
                                 description="custodial: empty cells that also close a sandwich (throne, corners).")
...
class CastleSpec(_Strict):
    """A castling move: royal `king` and `rook` both unmoved, cells between empty, the king not in check nor passing attacked cells."""  # noqa: E501 — the guide shows it as one line
    ...
    king_to: str
    rook_to: str
    name: str | None = Field(None, description="Notation (default: the king's move, e.g. e1-g1).")
...
class BoardConfig(_Strict):
    ...
    size: int | list[int] = Field(8, description="grid: n or [rows, cols]; hex: radius or [rows, cols]; ring: cells.")
    coords: Literal["algebraic", "rc"] = Field("algebraic",
                                               description="grid cell names: algebraic (a1 bottom-left) or rc (row,col "
                                                           "from the top-left).")
    ...
    cells: dict[str, dict[str, Any]] = Field(default_factory=dict,
                                             description="Cell properties: {cell: {color: dark, terrain: …}}; zones "
                                                         "can select them (\"color=dark\").")
    zones: dict[str, Zone] = Field(default_factory=dict,
                                   description="Named cell sets: a list, \"rank 8\", \"file a\", \"far\", \"near\", "
                                               "\"prop=value\", another zone, or {side: zone}.")
    sides: list[str | SideSpec] = Field(..., min_length=1,
                                        description="Player entity ids in seat order; the first moves first.")
    ...
    pieces: dict[str, PieceSpec] = Field(..., min_length=1,
                                         description="Piece kinds: "
                                                     "{kind: {name, symbol, moves, royal, promote, irreversible}}.")
    setup: str | dict[str, dict[str, list[str]]] = Field("",
                                                         description="Start position: grid rows top to bottom as "
                                                                     "board symbols (\"rnbqkbnr/pppppppp/8/…\"), or "
                                                                     "{side: {kind: [cells]}}.")
    hand: dict[str, dict[str, int]] = Field(default_factory=dict,
                                            description="Pieces in reserve at the start: {side: {kind: count}} (with "
                                                        "place.from hand).")
    place: PlaceSpec | None = Field(None,
                                    description="Players may place pieces on empty cells: "
                                                "{kinds, from, where, gravity}.")
    captures: list[CaptureSpec] = Field(default_factory=list,
                                        description="Capture rules after each move: custodial | flip | enclose.")
    castling: list[CastleSpec] = Field(default_factory=list,
                                       description="Castling moves: [{side, king, rook, king_to, rook_to, name}].")
    mandatory_capture: bool = Field(False, description="When a capture is available, only captures are legal.")
    self_check: bool = Field(False,
                             description="A move may not leave your royal piece attacked (check, checkmate, "
                                         "stalemate).")
    allow_pass: bool = Field(False, alias="pass", description="Players may pass (a `<name>_pass` tool).")
    no_moves: Literal["lose", "draw", "pass"] = Field("lose",
                                                      description="When the player to move has no legal move: lose "
                                                                  "(checkmate), draw, or pass automatically.")
    stalemate: Literal["draw", "lose"] = Field("draw",
                                               description="No legal move while the royal piece is not attacked.")
    line: int | None = Field(None, ge=2, description="Win by n of your pieces in a row.")
    lines: list[list[str]] = Field(default_factory=list,
                                   description="Explicit winning lines (cells) instead of straight rows.")
    repetition: int | None = Field(None, ge=2, description="Draw when the same position occurs this many times.")
    move_limit: int | None = Field(None, ge=1,
                                   description="Draw after this many moves in a row without a capture or irreversible "
                                               "move (100 = the 50-move rule).")
    passes_end: int | None = Field(None, ge=1,
                                   description="Consecutive passes that end the game (default: one per side).")
    score: Literal["none", "pieces", "area"] = Field("none",
                                                     description="Winner when the game ends by passes or when nobody "
                                                                 "can move: pieces on the board, or area (pieces + "
...
class Pattern:
    ...
    only: str | None = None
    first: bool = False
    origin: frozenset[int] | None = None
...
class Rules:
    ...
    place_zone: list[frozenset[int] | None] = field(default_factory=list)
    gravity: list[int] | None = None
    captures: tuple[CaptureRule, ...] = ()
    castles: tuple[Castle, ...] = ()
    lines: tuple[tuple[int, ...], ...] = ()
    chains: bool = False

    def flip_rule(self) -> CaptureRule | None:
        return next((c for c in self.captures if c.rule == "flip"), None)

    def enclose_rule(self) -> CaptureRule | None:
...
class _Compiler:
    ...
    def build(self) -> Rules:
        ...
        if c.line is None and c.lines:
            raise MechanismError("`lines` lists winning lines but `line` is not set", "set \"line\" to the line length",
                                 "line")
        return rules
    ...
    def _forward(self, side: int) -> str | None:
    ...
    def _pattern(self, spec: MoveSpec, side: int, forward: str | None, where: str) -> Pattern:
        geo = self.geo
    ...
    def _offsets(self, spec: MoveSpec, forward: str | None, where: str) -> list[tuple[int, int]]:
        raw = spec.leap or []
        ...
        if geo.shape not in ("grid", "hex"):
            raise MechanismError("leaps need a grid or hex board", "use step with a distance on rings and graphs",
                                 f"{where}.leap")
        ...
        for a, b in pairs:
            if geo.shape == "hex":
                ...
                if forward not in ("n", "s", "e", "w"):
                    raise MechanismError("exact leaps need an orthogonal forward direction",
                                         "set the side's forward to n, s, e or w", f"{where}.leap")
                fr, fc = {"n": (-1, 0), "s": (1, 0), "e": (0, 1), "w": (0, -1)}[forward]
                rr, rc = _right(forward)
                variants = [(a * fr + b * rr, a * fc + b * rc)]
            else:
                variants = sorted({(sa * x, sb * y) for x, y in set(permutations((a, b)))
        ...
        for side, spec in enumerate(self.specs):
            for kind in kinds:
                ...
                if isinstance(piece.symbol, dict):
                    ...
                    if unknown:
                        raise MechanismError(f"symbol for unknown side(s) {sorted(unknown)}",
                                             f"sides: {', '.join(self.sides)}", f"pieces.{kind}.symbol")
                ...
                if symbol is None or len(symbol) != 1 or symbol in ".0123456789/ ":
                    raise MechanismError(f"'{kind}' of {spec.id} needs a one-character symbol (not a digit, '.', '/')",
                                         "set the piece's symbol, e.g. "
                                         "{\"symbol\": {\"white\": \"o\", \"black\": \"x\"}}",
    ...
    def zone(self, spec: Zone | None, side: int, where: str, depth: int = 0) -> frozenset[int] | None:
        ...
        if isinstance(spec, dict):
            ...
            if unknown:
                raise MechanismError(f"zone for unknown side(s) {sorted(unknown)}", f"sides: {', '.join(self.sides)}",
                                     where)
...
def parse_setup(rules: Rules, spec: str | Mapping[str, Mapping[str, list[str]]],
    ...
    if isinstance(spec, Mapping):
        for side_id, kinds in spec.items():
            if side_id not in rules.sides:
                raise MechanismError(f"'{side_id}' is not a side", f"sides: {', '.join(rules.sides)}",
                                     f"{where}.{side_id}")
            if not isinstance(kinds, Mapping):
                raise MechanismError("give each side's pieces as {kind: [cells]}",
                                     "e.g. {\"white\": {\"K\": [\"e1\"]}}", f"{where}.{side_id}")
                ...
                if ch != ".":
                    ...
                    if piece is None:
                        raise MechanismError(f"row {r + 1}: '{ch}' is not a board symbol",
                                             f"symbols: {', '.join(sorted(rules.by_symbol))}; digits and '.' are empty "
                                             "cells", where)
                    if c < geo.cols:
    ...
    for _, _, index in out:
        if index in taken:
            raise MechanismError(f"two pieces are placed on {geo.names[index]}", "place at most one piece per cell",
                                 where)
        taken.add(index)
    return out
...
def _between(geo: Geometry, a: int, b: int) -> list[int] | None:
