...
an optional response stack (the ``decision`` family's ``procedure`` mode).
...
of the ``decision`` op (``{"decision": "trial", "action": "push", "item": "exhibit", ...}``), and
...
@mode("decision", "procedure", ProcedureConfig,
...
# ---------------------------------------------------------------------------
# The decision op's procedure actions
# ---------------------------------------------------------------------------
...
def _check_shared_completion(checker: Any, name: str, transition: Transition, path: str) -> None:
    ...
    if users is None:
        ...
        for owner, raw in checker.c.mechanisms.items():
            if raw.get("kind") != "decision" or raw.get("mode") != "procedure":
                continue
...
def _register_actions() -> None:
    for action, (keys, required, internal, needs, fields, doc) in _ACTIONS.items():
        example = ('{"decision": "trial", "action": "' + action + '"' + (f", {fields}" if fields else "")
                   + f"}}  ({doc})")
        family_action("decision", ("procedure",), action, keys=keys, required=required,
...
@function("stack(procedure, read?, ...)",
          ...
          min_args=1, max_args=4, family="decision")
