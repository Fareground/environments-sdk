...
class RolesConfig(BaseModel):
    ...
    teams: dict[str, list[str]] = Field(default_factory=dict,
                                        description="{team: [roles]}; a role in no team is its own team.")
    know: list[str] = Field(default_factory=list,
                            description="Teams (or roles) whose members know each other from the start.")
    ...
    reveal: Literal["elimination", "never"] = Field("elimination",
                                                    description="Reveal a role when its player is eliminated.")
    actions: dict[str, dict[str, Any]] = Field(
        default_factory=dict,
        description="Role-gated tools: {name: {...action, \"roles\": [roles]}}; private unless said otherwise.")
...
def _config_for(world: Any, entity: Entity) -> RolesConfig | None:
...
def _holders(runner: Any, effect: dict[str, Any], vars: dict[str, Any], where: str) -> tuple[RolesConfig, list[Entity]]:
    ...
    for player in players:
        if not runner.world.is_a(player.entity_type, config.who):
            raise RunError(f"{player.id} is a {player.entity_type}, not a {config.who} holding a role of {name}",
                           f"{where}.who")
    return config, players


@family_action("groups", ("roles",), "deal", internal=True,
               example='{"groups": "roles", "action": "deal"}  (shuffle the role deck and deal it; generated for round '
                       '1)')
...
@family_action("groups", ("roles",), "eliminate", keys=("who", "say"), required=("who",), templates=("say",),
               example='{"groups": "roles", "action": "eliminate", "who": "$out", "say": "{$out.name} is '
                       'exiled."}  (out of the game; the role is revealed unless reveal: never)')
...
def _reveal_op(runner: Any, effect: dict[str, Any], vars: dict[str, Any], where: str) -> None:
    ...
    if players:
        world.emit("revealed",
                   "Roles revealed: " + ", ".join(f"{p.name} was {_article(_props(p)['role'])}" for p in players) + ".")
...
def _teammates_function(call: Call) -> list[Entity]:
    ...
    return [q for q in world.entities_of(config.who)
            if q.id != player.id and _props(q).get("team") == _props(player).get("team")
...
def _expand_roles(name: str, config: RolesConfig, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for team, members in config.teams.items():
        ...
        for role in members:
            if role not in roles:
                raise MechanismError(f"team {team} lists '{role}', which is not in the deck",
                                     f"roles: {', '.join(roles)}", f"teams.{team}")
    ...
    for entry in config.know:
        if entry not in teams and entry not in roles:
            raise MechanismError(f"know lists '{entry}', which is neither a team nor a role",
                                 f"teams: {', '.join(teams)}", "know")
    alive = config.alive
    ...
    for action_name, raw in config.actions.items():
        ...
        if not allowed or unknown:
            raise MechanismError(f"a role-gated tool needs `roles` from the deck (got {gate!r})",
                                 f"roles: {', '.join(roles)}", f"actions.{action_name}.roles")
        who = " or ".join(allowed)
        spec["by"] = config.who
        spec["when"] = [{"expr": f"$actor.{alive}", "why": "You are out of the game."},
                        {"expr": f"$actor.role in [{', '.join(allowed)}]", "why": f"Only {_article(who)} can do this."}
                        ] + list(spec.get("when") or [])
    ...
    if config.views:
        ...
        fragment["views"] = {
            f"{name}_you": {"for": config.who, "title": "Your role",
                            "show": "You are {$actor.role|upper}{$' (team ' + $actor.team + ')' if $actor.team != "
                                    f"$actor.role else ''}}{out}."},
            f"{name}_team": {"for": config.who, "title": "Your team", "of": "$teammates($actor)",
                             "show": "[{id}] {name} — "
                                     f"{{$known_role($actor, $it)}}{{$'' if $it.{alive} else ' (out)'}}"},
