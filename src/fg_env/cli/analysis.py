...
def _sweep_params(items: list[str] | None) -> dict[str, Any]:
    if not items:
...
def _named_ranges(items: list[str] | None, flag: str) -> dict[str, dict[str, Any] | None]:
    if not items:
...
def cmd_sweep(args: argparse.Namespace) -> int:
    ...
    result = sweep(args.file, _sweep_params(args.param), runs=args.runs, outputs=args.output or None,
                   ...
                   participants=participants_arg(args.agent), rounds=args.rounds, seed=args.seed, workers=args.workers,
...
def cmd_sensitivity(args: argparse.Namespace) -> int:
    ...
    result = sensitivity(args.file, _named_ranges(args.vary, "--vary"), args.output, method=args.method,
                         ...
                         participants=participants_arg(args.agent), rounds=args.rounds, seed=args.seed,
                         workers=args.workers, data_dir=args.data_dir)
    _emit(result, args.json)
    return 0


def _held_out_cases(text: str | None) -> Any:
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    ...
    result = calibrate(args.file, goal, params, runs=args.runs, budget=args.budget, holdout=args.holdout,
                       method=args.method, inputs=_inputs(args), arm=args.arm,
                       participants=participants_arg(args.agent),
...
def _decisions(items: list[str] | None, path: str | None) -> dict[str, Any]:
...
def cmd_optimise(args: argparse.Namespace) -> int:
    ...
    result = optimise(args.file, _decisions(args.decision, args.decisions_file), objective, args.constraint or [],
                      ...
                      confidence=args.confidence, uncertainty=uncertainty, holdout_seeds=args.holdout_seeds,
                      inputs=_inputs(args), arm=args.arm,
                      participants=participants_arg(args.agent), rounds=args.rounds, data_dir=args.data_dir)
    _emit(result, args.json)
    return 0
...
def cmd_backtest(args: argparse.Namespace) -> int:
    ...
    result = backtest(args.file, cases, args.output, runs=args.runs, threshold=args.threshold, arm=args.arm,
                      participants=participants_arg(args.agent), rounds=args.rounds, seed=args.seed,
                      workers=args.workers, test=_held_out_cases(args.test), folds=args.folds, data_dir=args.data_dir)
    _emit(result, args.json)
    return 0
...
def _run_options(parser: argparse.ArgumentParser, seed_default: int | None = 0, workers: bool = True) -> None:
    ...
    parser.add_argument("--agent", action="append", metavar="[TYPE_OR_ID=]PARTICIPANT",
                        help="random | idle | policy:<name> | anthropic:<model> | openai:<model>, optionally for one "
                             "type or entity")
...
def _holdout_options(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument("--test", metavar="SHARE|NAMES",
                       help="hold out cases: a share (0.25) or comma-separated case names")
...
def _optimise_options(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--decision", action="append", metavar="NAME=LOW:HIGH[:STEP]|NAME=V1,V2|NAME={json}",
                   help="an input to decide (JSON for vectors: "
                        "{\"length\": 24, \"low\": 3, \"high\": 40, \"step\": 1})")
