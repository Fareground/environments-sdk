"""``fg-env legacy <command>``: the template API's command line (compile, lint, contract, packages, replay).

    fg-env legacy compile examples/tic_tac_toe/template.json --smoke 20
    fg-env legacy contract -o docs/kernel_contract.json
    fg-env legacy capabilities
"""
...
def cmd_new_primitive(args: argparse.Namespace) -> int:
    ...
    print("\nNext: edit the file, then run `fg-env legacy primitives` to verify it loaded.")
    return 0
...
def cmd_scaffold_env(args: argparse.Namespace) -> int:
    ...
    print(f"  next: edit, then `fg-env legacy compile {target}/template.json`")
    return 0
...
def add_legacy_command(sub: Any) -> None:
    """``fg-env legacy``: every template API command, one level down."""
    legacy = sub.add_parser("legacy", help="the earlier template API: compile, lint, contract, capabilities, versions, "
                                           "primitives, new-primitive, scaffold-env, pack, unpack, env-info, replay")
    sub = legacy.add_subparsers(dest="legacy_cmd", required=True, metavar="command")
