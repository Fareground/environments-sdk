...
class Trace:
    ...
    def __init__(self, source: TraceSource):
        ...
        if isinstance(source, Trace):
            ...
            raise TypeError(f"a trace is read from a RunResult, its dict, or a saved result file; got {type(source).__name__}")
    ...
    def text(self, digest: Optional[str]) -> Optional[str]:
    ...
    def turn(self, which: Union[int, str], round: Optional[int] = None) -> TraceView:
    ...
    def timeline(self, entity: Optional[str] = None) -> TraceView:
    ...
    def search(self, text: str) -> TraceView:
        ...
        for wake in self.wakes:
            for key in ("brief", "update"):
                shown = wake[key]
                if shown is not None and shown["hash"] in matching and (key != "brief" or
                                                                         (wake["entity"], shown["hash"]) not in reported):
    ...
    def invalid(self, entity: Optional[str] = None) -> TraceView:
    ...
    def _of(self, entity: str) -> List[Dict[str, Any]]:
        ...
        if not wakes:
            raise ValueError(f"'{entity}' was never woken in this run (agents: {', '.join(self.entities[:20]) or 'none'})")
        return wakes

    def _select(self, which: Union[int, str], round: Optional[int]) -> List[Dict[str, Any]]:
