...
class StageSpec(_Model):
    ...
    atomic: bool = Field(False, description="The turn's actions apply together or not at all: triggers, reactions and invariants wait until the turn ends, and a turn that breaks `valid` is undone. An action's own `outcome` text (and attached files) is shown once the turn commits, so an undone turn shows nothing it was not charged for.")
    valid: Annotated[List[Condition], BeforeValidator(one_or_many)] = Field(default_factory=list, description="Conditions the whole turn must meet when it ends ($actor, $pending); if one fails, every action of the turn is undone and the agent is told `why` and plays the turn again. An action that draws randomness settles the turn so far at once (a failure then undoes the turn and ends it), so no later action can undo its luck. Makes the stage atomic.")
...
class ViewSpec(_Model):
    ...
    look: bool = Field(False, description="Offer it on demand as look(view) instead of always including it. Randomness a view draws is fixed for the turn: looking again shows the same text.")
