...
def visible_key(env: Env, actor: Entity, pending: dict[str, Any]) -> str:
    """A key for the state with what ``actor`` cannot see left out: properties hidden from it (see world/hidden.py)
...
def _owned(env: Env, actor: Entity) -> set[str]:
    ...
    world/hidden.py): their private properties are not hidden from it."""
