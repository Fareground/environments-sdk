...
def _values(call: Call, value_index: int, where_index: Optional[int] = None) -> List[Any]:
...
def _variance(values: List[Any]) -> Optional[float]:
    if len(values) < 2:
        return None
    mean = _mean(values)
...
@function("variance(items, value?, where?)", "Sample variance (n - 1) of `value` over matching items, or of a list; null when fewer than two.",
          min_args=1, max_args=3, lazy=[1, 2])
def _variance_fn(call: Call) -> Optional[float]:
...
@function("cov(xs, ys)", "Sample covariance of paired lists (pairs with a null skipped); null when fewer than two pairs.",
          min_args=2, max_args=2)
def _cov(call: Call) -> Optional[float]:
    xs, ys = _pairs(call)
    if len(xs) < 2:
        return None
...
@function("corr(xs, ys)", "Pearson correlation of paired lists, -1 to 1; null when fewer than two pairs or a list is constant.",
          min_args=2, max_args=2)
def _corr(call: Call) -> Optional[float]:
    xs, ys = _pairs(call)
    if len(xs) < 2:
        return None
...
@function("linreg(xs, ys)", "Least-squares line through paired lists: {slope, intercept, r2, n}; null when fewer than two pairs or xs are constant.",
          min_args=2, max_args=2)
def _linreg(call: Call) -> Optional[Dict[str, Any]]:
    xs, ys = _pairs(call)
    if len(xs) < 2:
        return None
...
@function("gini(items, value?)", "Gini inequality of non-negative values, 0 (equal) to near 1 (one holds all); null when none.",
          min_args=1, max_args=2, lazy=[1])
def _gini(call: Call) -> Optional[float]:
...
@function("hhi(items, value?)", "Herfindahl–Hirschman concentration: the sum of squared shares, 1/n (even) to 1 (one holds all); null when none.",
          min_args=1, max_args=2, lazy=[1])
def _hhi(call: Call) -> Optional[float]:
...
@function("entropy(weights, base?)", "Shannon entropy of probabilities or counts (a list or map; normalised), in bits unless `base` is given; null when all are 0.",
          min_args=1, max_args=2)
def _entropy(call: Call) -> Optional[float]:
    raw = call.arg(0)
    values = list(raw.values()) if isinstance(raw, dict) else list_arg(call, 0, "a list or map of probabilities or counts")
...
@function("percentile_rank(values, x)", "Share of values below x, counting ties as half (0–1); nulls skipped, null when none.",
          min_args=2, max_args=2)
def _percentile_rank(call: Call) -> Optional[float]:
...
@function("zscore(x, values)", "Standard score (x - mean) / sample sd of the values; null when fewer than two values or no spread.",
          min_args=2, max_args=2)
def _zscore(call: Call) -> Optional[float]:
...
@function("autocorr(series, lag)", "Autocorrelation of a series with itself `lag` steps later; null when too short or constant.",
          min_args=2, max_args=2)
def _autocorr(call: Call) -> Optional[float]:
...
@function("returns(series, kind?)", "Step-to-step returns: simple (default) x[t] / x[t-1] - 1, or log ln(x[t] / x[t-1]); one shorter than the series.",
          min_args=1, max_args=2)
...
@function("ema(series, alpha)", "Exponential moving average, same length: e[0] = x[0], e[t] = alpha·x[t] + (1 - alpha)·e[t-1].",
          min_args=2, max_args=2)
...
@function("sma(series, n)", "Simple moving average over the last n items, same length (null until n items exist).",
          min_args=2, max_args=2)
def _sma(call: Call) -> List[Optional[float]]:
    values = series_arg(call, 0)
...
@function("drawdown(series)", "Largest fall from a running peak as a fraction of that peak (0–1), e.g. 0.25 for 120 → 90.",
...
@function("histogram(values, bins, low?, high?)", "Counts in `bins` equal-width bins over [low, high] (default the data range): {edges, counts}; outside values and nulls skipped.",
