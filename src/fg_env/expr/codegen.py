...
through :class:`~.calls.Call`, eagerly or per item) — becomes one Python function of the scope. Inside it
...
those helpers do. Syntax was whitelisted before compilation (see :mod:`.compile`).
...
from the call's own :class:`~.calls.Call` (so budget charges and equality guards are the same), and the
condition reads ``$it``, ``$i`` and ``$outer`` as loop variables and every other root from the caller's roots —
what :meth:`~.calls.Call.each` would give it — building the item's scope only for what needs one (a nested
call, a missing root).
"""
