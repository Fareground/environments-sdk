...
class Description:
    ...
    def info(self) -> str:
        ...
        rows = [
            ...
            ("chance",
             m["chance_mode"] + (f" (during {' and '.join(m['chance_during'])})" if m["chance_during"] else ""),
             "chance_mode"),
            ("information", m["information"], "information"),
            ("utility", m["utility"], "utility"),
            ("players", players, "players"),
            ("length", f"{_known(length['rounds'])} rounds at most, "
             + ("decisions not bounded" if length["decisions"] is None
                else f"{length['decisions']} decisions at most"),
             "max_game_length"),
            ("action space",
             space["kind"] + (f", {space['size']} distinct actions" if space["size"] is not None else ""),
             "action_space"),
        ]
        lines = [m["name"]]
...
def describe(contract: ContractLike, *, inputs: Mapping[str, Any] | None = None, arm: str | None = None,
...
def check_claims(contract: ContractLike, claims: Mapping[str, Any], *, inputs: Mapping[str, Any] | None = None,
                 arm: str | None = None, data_dir: Any = None, path: str = "game") -> list[Issue]:
