...
def odd_markdown(contract: C.Contract, metadata: Mapping[str, Any]) -> str:
    lines = [f"# {contract.name} — ODD description", "",
             "_Written from the contract by `fg_env.analysis.describe`, following the ODD protocol (Overview, Design "
             "concepts, Details). Participants — LLM agents, coded policies or people — are attached when the "
             "environment runs, so their decision making is outside this description._", ""]
...
def _entities(contract: C.Contract, metadata: Mapping[str, Any]) -> list[str]:
    ...
    if contract.relations:
        lines += ["### Relations", ""] + _table(["relation", "symmetric", "range", "link fields", "meaning"], [
            [name, "yes" if spec.symmetric else "no", _range(spec.min, spec.max), ", ".join(spec.props),
             spec.description]
            for name, spec in contract.relations.items()])
    if contract.records:
        lines += ["### Records (append-only logs)", ""] + _table(["record", "fields", "who can read an "
                                                                                      "entry", "keeps"], [
    ...
    if contract.space is not None:
        ...
        if contract.space.layers:
            lines.append("Cell layers: "
                         + ", ".join(f"`{name}` ({layer.type}{', ' + layer.description if layer.description else ''})"
...
def _concepts(contract: C.Contract, metadata: Mapping[str, Any]) -> list[str]:
    ...
    sensing += [f"entities of type `{kind}` can be inspected by {who}"
                for kind, who in observations.get("inspect", {}).items()]
    ...
    lines += ["**Sensing.** Agents read a static brief and a per-turn update in plain text, and act through typed "
              "tools.", ""]
    lines += _bullets(sensing, "")
    lines += ([f"**Information.** {metadata.get('information', 'unknown')}.", ""]
              + _bullets(evidence.get("information", []), ""))
    interaction = [f"`{name}` ({', '.join(spec.by if isinstance(spec.by, list) else [spec.by])}) targets "
                   ...
                   if any(ps.type == "entity" for ps in spec.params.values())]
    lines += (["**Interaction.** Actions that name other entities:", ""]
...
def _inputs(contract: C.Contract, metadata: Mapping[str, Any]) -> list[str]:
    ...
    if contract.feeds:
        lines += ["External data (feeds answered by host adapters; every answer is recorded on the host "
                  "tape):", ""] + _table(
...
def _submodels(contract: C.Contract, metadata: Mapping[str, Any]) -> list[str]:
    ...
    invariants = [f"`{i.expr}` ({when.get(i.check, i.check)})" + (f" — {i.why}" if i.why else "")
...
def _dynamics(contract: C.Contract) -> list[str]:
    ...
    lines += _table(["variable", "starts at", "rate", "noise", "range"], [
        [name, var.start, var.rate or "", var.noise or "", _range(var.min, var.max)]
        for name, var in physics.vars.items()])
    for kind, dynamics in physics.per.items():
        lines += [f"### Per-entity dynamics of `{kind}`" + (f" (where `{dynamics.where}`)" if dynamics.where else ""),
                  ""]
        lines += _table(["prop", "rate", "noise"],
                        [[name, var.rate, var.noise or ""] for name, var in dynamics.vars.items()])
        if dynamics.write:
...
def _patterns(contract: C.Contract) -> list[str]:
    ...
    if not rows:
        return []
    return ["### World patterns", "", "Regularities the world follows on its own, each read as a value wherever the "
            "rules need it:", ""] + _table(["pattern", "what it is", "kind", "per", "read as", "notes"], rows)
...
def _summary(metadata: Mapping[str, Any]) -> list[str]:
    ...
    rows: list[list[Any]] = [["dynamics", metadata.get("dynamics")], ["chance", metadata.get("chance_mode")],
            ...
            ["action space",
             f"{space.get('kind')}" + (f", at most {space['size']} distinct actions" if space.get("size") else "")],
