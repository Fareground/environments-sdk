...
- `form` (default "constant"): constant: the same chance at every age | weibull: rising (shape > 1) or falling (< 1) | loglogistic: rising then falling | table: one chance per age.
- `rate` (default 0.05): constant: chance per `span`.
- `shape` (default 1.0): weibull, loglogistic: the curve's shape.
- `scale` (default 10.0): weibull, loglogistic: typical age, in clock units.
- `values` (default null): table: chance at age 0, 1, 2 … (the last repeats).
- `span` (default 1.0): Clock units the chance covers (usually one round).
