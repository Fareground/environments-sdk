...
Every function by group. Read one group's signatures and docs with `guide('functions.<group>')` (a mechanism family's functions are also on its page).
...
- `text` (text, formatting and word games): $anagram $char_at $chars $contains $count_text $ends_with $fmt $index_of $join $lower $mask $matches $pad $repeat_text $replace $similar $split $starts_with $substr $text $title $trim $upper $wordle_feedback $words
...
- `game` (Game equipment: boards with enforced rules, cards, betting pots and worker-placement slots): $blackjack_soft $blackjack_value $board_at $board_cell $board_in_check $board_line $board_moves $board_render $board_score $card_names $card_visible $cards_table $claims $follow_suit $hand $open_spaces $poker_hand $poker_rank $pot_live $pot_options $pot_table $pot_total $puzzle $runs $sets $slot_board $solve $top_card $top_cards $trick_winner $zone
...
- `groups` (Who belongs with whom: hidden roles and teams, factions and alliances, relationships): $allies $faction_of $factions $joinable $known_role $team_alive $teammates
