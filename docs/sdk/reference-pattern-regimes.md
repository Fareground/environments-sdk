...
- `states` (required): {state: value}: what each state gives (a number, or a map read as $pattern.economy.growth); null gives the state's name.
- `transitions` (required): {from: {to: probability per step}}; the rest of the probability stays in the state.
- `start` (default null): The state at round 1 (default: the first declared).
...
```json
{"types": {}, "mechanisms": {"my_regimes": {"kind": "pattern", "mode": "regimes", "states": {"boom": {"growth": 0.02}, "bust": {"growth": -0.01}}, "transitions": {"boom": {"bust": 0.05}, "bust": {"boom": 0.2}}}}}
```
