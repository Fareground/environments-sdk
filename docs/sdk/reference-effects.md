...
- `after`: {"after": 3, "do": [...]}  (runs 3 rounds later with the same locals; scheduled by an action, it stays that action's: a refusal or failing rule in it undoes that block alone and tells the agent, and the run goes on; if that agent has left the run by then, the block does not run at all — the diagnostic `after_dropped` counts it; schedule it from an event to have it run regardless)
