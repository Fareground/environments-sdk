...
- `after`: {"after": 3, "do": [...]}  (runs 3 rounds later with the same locals; scheduled by an action, it stays that action's: a refusal or failing rule in it undoes that block alone and tells the agent, and the run goes on)
