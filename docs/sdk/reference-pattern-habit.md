...
- `form` (default "habit"): habit: 1 + strength·S/(1 + S), growing with repetition | fatigue: 1/(1 + strength·S), wearing out with exposure.
- `strength` (required): How strongly the remembered exposure S acts.
- `retain` (default 0.8): Share of S kept each step.
- `half_life` (default null): Steps for S to halve (instead of retain).
