...
- `apply` — takes `status`, `who`, `rounds`, `stacks`, `source` (needs `status`, `who`): {"conditions": "conditions", "action": "apply", "status": "poison", "who": "$params.target", "rounds": 3, "stacks": 1}  (rounds and stacks are optional; source defaults to $actor)
- `cleanse` — takes `status`, `who` (needs `status`, `who`): {"conditions": "conditions", "action": "cleanse", "status": "poison", "who": "$params.ally"}  (a status, a list, or "all" for every cleansable one; on_expire does not run)

```json
{"mechanisms": {"my_status": {"kind": "conditions", "mode": "status", "who": "unit", "statuses": {"poison": {"duration": 3, "max_stacks": 3, "tick": ["$it.hp -= 2 * $stacks"]}, "stun": {"duration": 1, "blocks": ["attack"], "blocked_why": "you are stunned"}, "shield": {"duration": 2, "modifiers": {"armor": 3}, "immune": ["poison"]}}}}}
```
