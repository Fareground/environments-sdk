...
- `first` (required): Cost (or time) of the first unit.
- `rate` (default 0.8): Progress ratio: each doubling of cumulative units multiplies the cost by it.
- `floor` (default 0.0): Lowest it gets.
...
```json
{"types": {}, "mechanisms": {"my_learning_curve": {"kind": "pattern", "mode": "learning_curve", "first": 120, "rate": 0.85, "floor": 40}}}
```
