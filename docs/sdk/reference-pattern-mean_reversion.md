...
- `mean` (required): The level it is pulled back to.
- `rate` (required): Pull per clock unit (0.1: a gap shrinks by e^−0.1 ≈ 10% a unit).
- `sd` (default 0.0): Noise per √unit (the long-run spread is sd / √(2·rate)).
- `start` (default null): Value at round 1 (default: the mean).
