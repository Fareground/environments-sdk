...
- `form` (default "linear"): linear: start + slope·t | exponential: start·e^(rate·t) | logistic: capacity / (1 + e^(−steepness·(t − midpoint))).
- `start` (default 1.0): Value at `origin` (linear, exponential).
- `slope` (default 0.0): Change per clock unit (linear).
- `rate` (default 0.0): Growth per clock unit, as a log rate: 0.01 ≈ +1% a unit (exponential).
- `capacity` (default 1.0): The level it saturates at (logistic).
- `midpoint` (default 0.0): Clock units after `origin` when it is half way (logistic).
- `steepness` (default 1.0): How fast it rises around the midpoint (logistic).
- `origin` (default 0.0): Where t counts from: clock units from round 1, or an ISO date.
...
```json
{"inputs": {"growth": {"type": "number", "default": 0.02}}, "types": {}, "mechanisms": {"my_trend": {"kind": "pattern", "mode": "trend", "form": "exponential", "start": 100, "rate": "$inputs.growth"}}}
```
