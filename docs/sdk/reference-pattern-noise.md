...
- `dist` (default "normal"): The distribution of each step's draw.
- `mean` (default 0.0): Centre (normal, laplace); log-mean (lognormal).
- `sd` (default 1.0): Spread (normal, laplace: scale·√2; lognormal: log-sd).
- `low` (default 0.0): Lowest value (uniform).
- `high` (default 1.0): Highest value (uniform).
...
```json
{"types": {"resident": {}}, "entities": {"resident": {"type": "resident", "count": 2}}, "mechanisms": {"my_noise": {"kind": "pattern", "mode": "noise", "dist": "normal", "sd": 0.01, "keys": "resident"}}}
```
