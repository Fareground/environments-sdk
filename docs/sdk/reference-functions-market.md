...
- `$auction(name?)` — An auction's state (the name may be left out while the contract has one auction): {format, open, lot, price, leader, min_bid, reserve, bids, sold, revenue, stock, items, last}. price, leader and bids describe the open lot; items what a combinatorial lot still has for sale; last the latest closed lot's result, sold or not: {lot, winner, winners, price, qty, note} (winner: the first winner's id, '' when unsold; price: the first winner's price per unit), kept until another lot closes, null before any has.
