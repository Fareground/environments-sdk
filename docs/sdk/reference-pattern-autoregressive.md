...
- `coefficients` (required): [φ1, φ2, …]: how much each earlier step carries into the next.
- `mean` (default 0.0): The level deviations are measured from.
- `sd` (default 1.0): Standard deviation of each step's new shock.
- `start` (default null): Value of the first steps (default: the mean).
