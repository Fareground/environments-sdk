...
- `period` (required): Clock units per cycle (a business cycle of 20 weeks: 20).
- `amplitude` (default 1.0): Height above and below the level.
- `level` (default 0.0): The centre it swings around.
- `phase` (default 0.0): Share of a cycle it is shifted later, from 0 to 1.
- `shape` (default "sine"): The wave's shape.
...
```json
{"types": {}, "mechanisms": {"my_cycle": {"kind": "pattern", "mode": "cycle", "period": 26, "amplitude": 0.1, "level": 1}}}
```
