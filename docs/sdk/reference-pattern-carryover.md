...
- `retain` (default 0.5): Share carried into the next step (adstock decay).
- `half_life` (default null): Steps for a past input's effect to halve (instead of retain).
- `lag` (default 0): Steps before an input starts to count.
- `form` (default "sum"): sum: input + retain·stock (adstock) | average: (1 − retain)·input + retain·stock (a moving average).
- `start` (default 0.0): The stock before round 1 (and the input before a lag has filled).
...
```json
{"world": {"ad_spend": 100.0}, "types": {}, "mechanisms": {"my_carryover": {"kind": "pattern", "mode": "carryover", "input": "$world.ad_spend", "half_life": 2, "lag": 1}}}
```
