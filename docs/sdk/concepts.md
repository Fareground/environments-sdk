...
- A refusal tells the actor something. A refusal from `do` (a `fail`, a transfer that cannot be covered) spends the action, so a hidden value costs a guess per try; a failed `when` costs nothing, so a `when` that reads another agent's private state (however it reaches it) is a check warning. If the rule itself should stay secret, accept the action and settle the hidden part in `do`.
