...
- A refusal tells the actor something. A refusal from `do` that read a private property of another entity (a `fail` on a hidden code) spends the action, so a hidden value costs a guess per try; any other refusal (a taken cell, bad arguments, a failed `when`) costs nothing, so a `when` that reads another agent's private state (however it reaches it) is a check warning. If the rule itself should stay secret, accept the action and settle the hidden part in `do`.
