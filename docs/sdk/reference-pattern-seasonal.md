...
- `period` (default "year"): year, quarter, month, week, day, hour — or a number of clock units. With clock.start, year, week and day follow the calendar.
- `profile` (default null): One value per slot of the period: 12 over a year are calendar months, 7 over a week weekdays (Monday first), 24 over a day hours; other counts are equal slices.
- `amplitude` (default 0.0): Height of a smooth yearly-style wave (0.2 = ±20% with form multiply).
- `peak` (default 0.0): Where in the period the wave peaks, from 0 to 1 (0.5 = the middle).
- `harmonics` (default null): [[sin, cos], …]: the k-th pair is a wave k times per period (fitted by harmonic regression).
- `form` (default "multiply"): multiply: an index around 1 (profile × (1 + waves)) | add: an amount around 0 (profile + waves).
...
```json
{"mechanisms": {"my_seasonal": {"kind": "pattern", "mode": "seasonal", "period": "year", "profile": [0.8, 0.8, 0.9, 1, 1.1, 1.2, 1.3, 1.2, 1.1, 1, 0.9, 0.7]}}}
```
