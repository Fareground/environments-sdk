...
- `maker_fee_bps` (default 0.0): Fee on fills of resting orders, in basis points of notional (number or expression over $inputs, resolved when the world is built).
...
- `volatility` (default 0.02): Per-round return volatility coded strategies assume before the tape shows one (number or expression).
...
- `fair_value` (default null): Expression for the true value fundamentalists estimate (default: the start price).
