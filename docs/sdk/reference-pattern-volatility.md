...
- `omega` (required): Baseline variance added each step (> 0).
- `alpha` (default 0.1): How much the last return's size raises the next variance.
- `beta` (default 0.85): How much the last variance carries over (alpha + beta < 1 is stable).
- `mean` (default 0.0): Mean return per step.
- `start` (default 100.0): Level at round 1 (output level).
- `output` (default "returns"): returns: each step's log return | level: start compounded by the returns | volatility: the standard deviation now.
