...
- `elasticity` (required): % change in quantity per % change in price at the reference (−1.5).
- `reference` (default 1.0): The price where the effect is 1.
- `form` (default "constant"): constant: (price/reference)^elasticity | linear: 1 + elasticity·(price/reference − 1), never below 0.
...
```json
{"mechanisms": {"my_elasticity": {"kind": "pattern", "mode": "elasticity", "elasticity": "$inputs.elasticity", "reference": 24.99}}}
```
