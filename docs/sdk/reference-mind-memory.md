...
- `note` — takes `text` (needs `text`): {"mind": "memory", "action": "note", "text": "$params.text"}  (add a note to the actor's memory)
- `recall` — takes `query` (needs `query`): {"mind": "memory", "action": "recall", "query": "$params.query"}  (the actor's most relevant memories as text in $actor.memory_recalled; recalled memories strengthen)

```json
{"mechanisms": {"my_memory": {"kind": "mind", "mode": "memory", "who": "panelist", "half_life": 5, "budget": 250}}}
```
