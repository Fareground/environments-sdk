...
- `form` (default "power"): power: base + strength·users^exponent | log: base + strength·ln(1 + users).
- `strength` (required): How much users add.
- `exponent` (default 1.0): power: 1 linear, 2 Metcalfe-like, < 1 diminishing.
- `base` (default 1.0): Value with no users.
...
```json
{"types": {}, "mechanisms": {"my_network": {"kind": "pattern", "mode": "network", "form": "log", "strength": 0.2}}}
```
