...
gives each attached mechanism the actions per turn it has in its own stage, and one more for your own actions when it offers any. A mechanism with no `stage` gets a stage of its own, which runs after your stages, in the order the mechanisms are declared. Combine
