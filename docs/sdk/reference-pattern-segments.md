...
- `segments` (required): {segment: {share, values}}: each key falls in one segment, drawn once per run; reads give {segment, …values}.
...
**Segment** — 
- `share`: number | text (required) — Relative share of keys in this segment.
